...
frappe.ui.form.on("Hotel Room", "refresh", function(frm, cdt, cdn) {
	...
	frappe.call({
		...
		callback: (response) => {
			for (let i = 0; i < response.message.length; i++) {
				...
				if (is_viewable) {
					// html = html + '<div class="img_preview"><img class="img-responsive" src="'+response.message[i].file_url+'"></img></div><br>';
