...
def update_by_reservation(reservation_name):
	...
	if status == 'Cancel':
		for reservation_detail in reservation_detail_list:
			frappe.db.sql(
				'UPDATE `tabRoom Booking` SET status="Canceled" WHERE reference_type="Reservation Detail" AND reference_name=%s',
				(reservation_detail.name))
	elif status == 'Finish':
		for reservation_detail in reservation_detail_list:
			frappe.db.sql(
				'UPDATE `tabRoom Booking` SET status="Finished" WHERE reference_type="Reservation Detail" AND reference_name=%s',
				(reservation_detail.name))
	else:
		for reservation_detail in reservation_detail_list:
			room_booking_name = frappe.db.get_value('Room Booking',
													{'reference_type': 'Reservation Detail',
													 'reference_name': reservation_detail.name},
													['name']
													)
			...
			if room_booking_name is None:
				...
				if not stayed:
					frappe.db.sql('UPDATE `tabRoom Booking` SET start=%s, end=%s, room_id=%s, room_availability="RS", '
								  'note="", status="Booked" WHERE name=%s', (reservation_detail.expected_arrival,
																			 reservation_detail.expected_departure,
																			 reservation_detail.room_id,
																			 room_booking_name))

def update_by_room_stay(room_stay_id):
	room_stay = frappe.get_doc('Room Stay', room_stay_id)
	reservation_detail = frappe.get_doc('Reservation Detail',
										{'parent': room_stay.reservation_id, 'room_id': room_stay.room_id})
				...
				if filters.get('room_type'):
					if filters.get('bed_type'):
						room_list = list(frappe.db.sql(
							"select name, room_type, bed_type, allow_smoke from `tabHotel Room` where room_status = 'Vacant Ready' and allow_smoke = %s and room_type = %s and bed_type = %s",
							(filters.get('allow_smoke'), filters.get('room_type'), filters.get('bed_type'))))
					else:
						room_list = list(frappe.db.sql(
							"select name, room_type, bed_type, allow_smoke from `tabHotel Room` where room_status = 'Vacant Ready' and allow_smoke = %s and room_type = %s",
							(filters.get('allow_smoke'), filters.get('room_type'))))
				else:
					room_list = list(frappe.db.sql(
						"select name, room_type, bed_type, allow_smoke from `tabHotel Room` where room_status = 'Vacant Ready' and allow_smoke = %s",
						(filters.get('allow_smoke'))))
			else:
				room_list = list(frappe.db.sql(
					"select name, room_type, bed_type, allow_smoke from `tabHotel Room` where room_status = 'Vacant Ready'"))
	else:
		if filters.get('start') and filters.get('end'):
			if filters.get('allow_smoke'):
				if filters.get('room_type'):
					if filters.get('bed_type'):
						room_list = list(frappe.db.sql(
							"select name, room_type, bed_type, allow_smoke from `tabHotel Room` where allow_smoke = %s and room_type = %s and bed_type = %s",
							(filters.get('allow_smoke'), filters.get('room_type'), filters.get('bed_type'))))
					else:
						room_list = list(frappe.db.sql(
							"select name, room_type, bed_type, allow_smoke from `tabHotel Room` where allow_smoke = %s and room_type = %s",
							(filters.get('allow_smoke'), filters.get('room_type'))))
				else:
					room_list = list(frappe.db.sql(
						"select name, room_type, bed_type, allow_smoke from `tabHotel Room` where allow_smoke = %s",
						(filters.get('allow_smoke'))))
			else:
...
def is_available(room_id, start, end, name):
	if name:
		result = frappe.db.sql(
			'SELECT room_id FROM `tabRoom Booking` WHERE %s != name AND %s = room_id AND status = "Booked" AND (start != end) AND ((%s >= start AND %s < end) OR (%s > start AND %s <= end) OR (%s < start AND %s > end))',
			(name, room_id, start, start, end, end, start, end))
	else:
		result = frappe.db.sql(
			'SELECT room_id FROM `tabRoom Booking` WHERE %s = room_id AND status = "Booked" AND (start != end) AND ((%s >= start AND %s < end) OR (%s > start AND %s <= end) OR (%s < start AND %s > end))',
...
def get_room_booking(room_id, date):
	return frappe.db.sql(
		'SELECT name, start, end, room_availability, note FROM `tabRoom Booking` WHERE status = "Booked" AND room_id = %s AND %s >= start AND %s < end',
		(room_id, date, date))
